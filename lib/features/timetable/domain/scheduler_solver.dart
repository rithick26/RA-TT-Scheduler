...
class SchedulerSolver {
  ...
  void validate() {
    ...
    for (int year = 1; year <= 4; year++) {
      ...
      final totalHours = yearSubjects.fold<int>(0, (sum, s) => sum + (hoursPerWeek[s.id] ?? 0));
      if (totalHours > 40) {
        throw SchedulerException(
    ...
    for (final subject in subjects) {
      ...
      facultyHours[subject.faculty1Id] = (facultyHours[subject.faculty1Id] ?? 0) + hours;
      ...
      if (subject.faculty2Id != null) {
        facultyHours[subject.faculty2Id!] = (facultyHours[subject.faculty2Id!] ?? 0) + hours;
      ...
      if (subject.faculty3Id != null) {
        facultyHours[subject.faculty3Id!] = (facultyHours[subject.faculty3Id!] ?? 0) + hours;
    ...
    for (final facultyId in facultyHours.keys) {
      ...
      if (hours > 40) {
        final name = facultyNames[facultyId] ?? "Unknown Faculty (ID: $facultyId)";
        throw SchedulerException(
  ...
  List<TimetableSlot>? _trySolve(Random rand, int maxIterations) {
    ...
    final grid = List.generate(4, (_) => List.generate(5, (_) => List<Subject?>.filled(8, null)));
    ...
    final labs = subjects.where((s) => s.subjectType.toLowerCase() == 'lab' && (hoursPerWeek[s.id!] ?? 0) > 0).toList();
    // Sort labs by hours descending to place larger labs first (heuristics)
    labs.sort((a, b) => (hoursPerWeek[b.id!] ?? 0).compareTo(hoursPerWeek[a.id!] ?? 0));

    // List of theories/projects/others to schedule
    final theories = subjects.where((s) => s.subjectType.toLowerCase() != 'lab' && (hoursPerWeek[s.id!] ?? 0) > 0).toList();
    ...
    int iterations = 0;
    bool success = _scheduleTheories(grid, theories, remainingHours, 0, 0, 0, rand, () {
      iterations++;
  ...
  bool _scheduleLabs(
    ...
    for (int d = 0; d < 5; d++) {
      for (int p = 0; p <= 8 - hours; p++) {
        // Labs usually don't cross lunch breaks (e.g. if lunch is after period 4, we might avoid crossing period 3 to 4,
        // but let's just allow any block within the 8 periods as a baseline, or prioritize forenoon/afternoon)
        candidates.add(MapEntry(d, p));
      }
    }
  ...
  bool _scheduleTheories(
    ...
    if (grid[yearIdx][d][p] != null) {
      return _scheduleTheories(grid, theories, remaining, yearIdx, d, p + 1, rand, isTimeout);
    }

    final year = yearIdx + 1;
    // Get candidate subjects for this year that still have remaining hours
    final candidates = theories.where((s) => s.year == year && remaining[s.id!]! > 0).toList();
    ...
    for (final s in candidates) {
      if (_canPlaceTheory(grid, s, d, p, yearIdx, remaining)) {
        ...
        if (_scheduleTheories(grid, theories, remaining, yearIdx, d, p + 1, rand, isTimeout)) {
          return true;
        }
  ...
  bool _canPlaceTheory(
    ...
    final weeklyHours = hoursPerWeek[subject.id!] ?? 0;
    final maxPerDay = (weeklyHours <= 5) ? 2 : 3;
    ...
    // 3. Avoid more than 2 consecutive periods of the same theory subject
    if (p >= 2) {
      if (grid[yearIdx][d][p - 1]?.id == subject.id &&
          grid[yearIdx][d][p - 2]?.id == subject.id) {
        return false;
      }
    }
    ...
    for (int y = 0; y < 4; y++) {
      ...
      if (otherSubject != null) {
        ...
        if (teachers.contains(otherSubject.faculty1Id) ||
            (otherSubject.faculty2Id != null && teachers.contains(otherSubject.faculty2Id!)) ||
            (otherSubject.faculty3Id != null && teachers.contains(otherSubject.faculty3Id!))) {
          return true;
        }
      }
    }
